...
class APIClient {
  ...
  // Search endpoints
  async semanticSearch(body, token) {
  ...
  async hybridSearch(body, token) {
    return this.makeRequest('/search/hybrid', {
      method: 'POST',
      body: JSON.stringify(body),
      token,
    });
...
export const useApiClient = () => {
  ...
  const wrappedClient = {
    async semanticSearch(body) {
      ...
      return client.semanticSearch(body, token);
    },

    async hybridSearch(body) {
      const token = await getTokenSafely();
      return client.hybridSearch(body, token);
    },
    async getAnalytics() {
      const token = await getTokenSafely();
